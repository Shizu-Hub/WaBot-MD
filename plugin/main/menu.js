    ...
    try {
      if(q){
        ...
        teks = global.footer + " *[ Beta✓ ]*\n\n"
		  	teks += monospace(" ❏ Creador del Bot : " + "@" + config.owner[0].split("@")[0] )+ "\n"
        ...
        for (let cmd of cmds){
          let info = command.get(cmd);
			...
			menu += monospace(" ❏ Creador del Bot  : " + "@" + config.owner[0].split("@")[0] )+ "\n"
			menu += monospace(" ❏ Prefix : [ " + pref + " ]") + "\n"
			menu += monospace(" ❏ fecha : " + date) + "\n"
		  menu += monospace(" ❏ velocidad :  " + processTime(msg.messageTimestamp, moment()) + " Seccond") + "\n\n"
			...
			menu += `_Nota: Envia ${prefix}help <comando> para ver información de comando_`
